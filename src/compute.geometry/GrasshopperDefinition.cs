    ...
    class GrasshopperDefinition
    {
        ...
        public void SetInputs(Schema inputSchema)
        {
            if(inputSchema.DataFormat == SchemaDataFormat.Grasshopper)
            {
                ...
                {
                    if (!_input.TryGetValue(entry.Key, out var inputGroup))
                    {
                        continue;
                    }
        ...
        static readonly System.Collections.Concurrent.ConcurrentDictionary<Type, MethodInfo> _assignContextualDataTreeMethods =
